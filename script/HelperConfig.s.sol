...
contract HelperConfig is Script,CodeConstants {
    ...
    function getMainnetConfig() public pure returns (NetworkConfig memory networkConfig) {
        ...
        networkConfig = NetworkConfig({wBtc:wrappedBtc,wEth:wrappedEth,ethUsd:EthForUsd,btcUsd:BtcForUsd,deployerAccount:account});
    ...
    function createAndGetAnvilConfig() public  returns (NetworkConfig memory networkConfig){
        ...
        networkConfig = NetworkConfig({wBtc:address(wrappedBtc),wEth:address(wrappedEth),ethUsd:address(ethPriceFeed),btcUsd:address(btcPriceFeed),deployerAccount:DEFAULT_SENDER });
    }
